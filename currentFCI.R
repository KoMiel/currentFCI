...
# a function that performs bootstrapping for currentFCI

bootstrap <- function(data, nBoot, pBoot, indepTest, alpha,
                      partners, posOut, posMisc,
                      doPdsep, conservative, m.max,
                      posBio = NULL, correlation = "pearson") {
  
  # generate a list of all nodes
  nodes <- colnames(data)
  
  # generate matrices for the different potential edge mark types
  # 1: circle, 2: arrowhead, 3: tail
  Mat1 <- matrix(ncol = ncol(data), nrow = ncol(data), 0)
  rownames(Mat1) <- nodes
  colnames(Mat1) <- nodes
  
  Mat2 <- matrix(ncol = ncol(data), nrow = ncol(data), 0)
  rownames(Mat2) <- nodes
  colnames(Mat2) <- nodes
  
  Mat3 <- matrix(ncol = ncol(data), nrow = ncol(data), 0)
  rownames(Mat3) <- nodes
  colnames(Mat3) <- nodes
  ...
  for (run in 1:nBoot) {
    ...
    # sample the requested amount of data
    dataSet <- data[sample(nrow(data), size = round(pBoot*nrow(data)), replace = TRUE),]
    dataSet <- as.matrix(dataSet)
    dataSet <- list(C = cor(dataSet, method = correlation), n = nrow(dataSet))
    
    # apply the algorithm
    result <- currentFci(dataSet, indepTest, alpha, rownames(dataSet$C), partners = partners, posOut = posOut, posRem = posMisc, doPdsep = doPdsep, conservative = conservative, m.max = m.max, posBio = posBio)

    # get the causal matrix
    mat <- attributes(result)$amat
    
    # count
    Mat1 <- Mat1 + as.numeric(mat == 1)
    Mat2 <- Mat2 + as.numeric(mat == 2)
    Mat3 <- Mat3 + as.numeric(mat == 3)
  }
  
  # data frame to store the results
  connectionTable <- data.frame(matrix(ncol = 4, nrow = 3 * length(nodes) * length(nodes), 0))
  
  # set the names of the data frame columns
  names(connectionTable) <- c("Node1", "Node2", "Edgemark", "Count")
  
  # counter
  k <- 1
  
  # loop over matrices and store all results in data frame
  for (i in 1:nrow(Mat1)) {
    for (j in 1:ncol(Mat1)) {
      connectionTable[k, ] <- c(nodes[i], nodes[j], 1, Mat1[i, j])
      k <- k + 1
      connectionTable[k, ] <- c(nodes[i], nodes[j], 2, Mat2[i, j])
      k <- k + 1
      connectionTable[k, ] <- c(nodes[i], nodes[j], 3, Mat3[i, j])
      k <- k + 1
    }
  }
  
  # convert to integer
  connectionTable["Count"] <- as.integer(connectionTable[,"Count"])
  
  # remove entries without a single count
  connectionTable <- connectionTable[connectionTable['Count'] > 0,]
  
  # sort by the count
  connectionTable <- connectionTable[order(connectionTable['Count'], decreasing = TRUE),]
...
# currentFCI, build on pcalgs fci function
# changes are indicated in the code

currentFci <- function(suffStat, indepTest, alpha, labels, p,
                    ...
                    partners, posOut, posRem, posBio = NULL) # additional variables
{
  ...
  # first, learn a skeleton for the instream variables only
  # this is then used for the upstream part of the causal network
  gaps <- matrix(nrow = nrow(suffStat$C), ncol = ncol(suffStat$C), TRUE)
  gaps[partners[,2], partners[,2]] <- FALSE
  
  skelIn <- skeleton(suffStat, indepTest, alpha, labels = labels, method = skel.method,
                     fixedGaps = gaps, fixedEdges = fixedEdges,
                     NAdelete=NAdelete, m.max=m.max, numCores=numCores, verbose=verbose)
  sepsetIn <- skelIn@sepset
  GIn <- as(skelIn@graph, "matrix")
  
  # fix the learned structure upstream
  edges <- matrix(nrow = nrow(suffStat$C), ncol = ncol(suffStat$C), FALSE)
  edges[partners[,1], partners[,1]] <- GIn[partners[,2], partners[,2]] == 1
  
  # fix edges for counterpart variables
  for (i in 1:nrow(partners)) {
    edges[partners[i,1], partners[i,2]] <- TRUE
    edges[partners[i,2], partners[i,1]] <- TRUE
  }
  
  # remove the already eliminated edges upstream and instream
  gaps <- matrix(nrow = nrow(suffStat$C), ncol = ncol(suffStat$C), FALSE)
  gaps[partners[,1], partners[,1]] <- GIn[partners[,2], partners[,2]] == 0
  gaps[partners[,2], partners[,2]] <- GIn[partners[,2], partners[,2]] == 0
  
  # skeleton phase
  skel <- skeleton(suffStat, indepTest, alpha, labels = labels, method = skel.method,
                   fixedGaps = gaps, fixedEdges = edges,
                   NAdelete=NAdelete, m.max=m.max, numCores=numCores, verbose=verbose)
  G <- as(skel@graph, "matrix")
  G[partners[,1], partners[,1]] <- 0
  skel@graph <- as(G, "graphNEL")
  
  # if two nodes were seperated in the first skeleton phase already, we have to copy the sepset to have a full set of them
  sepset <- skel@sepset
  for (i in 1:length(sepset)) {
    for (j in 1:length(sepset[[i]])) {
      if (is.null(sepset[[i]][[j]]) & !is.null(sepsetIn[[i]][[j]])) {
        sepset[[i]][[j]] <- sepsetIn[[i]][[j]]
      }
    }
  }
  skel@sepset <- sepset
  
  # sum the number of tests of both skeleton phases
  length(skelIn@n.edgetests) <- length(skel@n.edgetests)
  skel@n.edgetests <- rowSums( cbind (skel@n.edgetests,skelIn@n.edgetests), na.rm=TRUE)
  
  # get the correct pMax and max.ord values
  skel@pMax <- pmax(skel@pMax, skelIn@pMax)
  skel@max.ord <- max(skel@max.ord, skelIn@max.ord)
  ...
  if (doPdsep) {
    ...
    # search for unfaithful triples in instream skeleton
    pc.ciIn <- pc.cons.intern(skelIn, suffStat, indepTest,
                              alpha = alpha, version.unf = c(1,1),
                              maj.rule = FALSE, verbose = verbose)
    
    # we first call a modified version of pdsep for the upstream part; this ONLY orients the colliders and returns the adjacency matrix
    pdsepResIn <- pdsep(skelIn@graph, suffStat, indepTest = indepTest, p = p,
                        sepset = pc.ciIn$sk@sepset, alpha = alpha, pMax = pMax,
                        m.max = if (type == "adaptive") max.ordSKEL else m.max,
                        pdsep.max = pdsep.max, NAdelete = NAdelete,
                        unfVect = pc.ciIn$unfTripl, # "tripleList.pdsep"
                        biCC = biCC, verbose = verbose)

    # rename
    pdsepResUp <- pdsepResIn
    
    # replace instream variables with upstream counterparts
    if(!is.null(nrow(pdsepResIn))) {
      pdsepResUp[partners[,1], partners[,1]] <- pdsepResIn[partners[,2], partners[,2]]
      pdsepResUp[partners[,2], partners[,2]] <- 0
    } else {
      pdsepResUp <- matrix(nrow = nrow(G), ncol = ncol(G), 0)
      rownames(pdsepResUp) <- rownames(G)
      colnames(pdsepResUp) <- colnames(G)
    }
    
    # insert upstream graph
...
#        mat[partners[i,1], partners[i,2]] <- 2
    ...
    # additional unfaithful triples: those in which we know the direction from background knowledge (partner variables), but the rules say different
    tripleList <- triples(mat, pc.ci$sk@sepset, pc.ci$unfTripl)
    
    # now we call the full pdsep function which takes the adjacency matrix that we learned for the upstream part as additional input
    # we use the upstream structure to test the other connections, but do not test the connections themselves
    pdsepRes <- pdsep(mat, suffStat, indepTest = indepTest, p = p,
                      sepset = pc.ci$sk@sepset, alpha = alpha, pMax = pMax,
                      m.max = if (type == "adaptive") max.ordSKEL else m.max,
                      pdsep.max = pdsep.max, NAdelete = NAdelete,
                      unfVect = tripleList, # "tripleList.pdsep"
                      biCC = biCC, verbose = verbose, amatUp = pdsepResUp)
        
    # we are not interested in the orientations upstream; they were only learned to be able to find the correct separating sets -> remove them
    pdsepRes$G[partners[,1], partners[,1]] <- FALSE
  ...
  # orient from background knowledge
  G[posOut, partners[,2]][G[posOut, partners[,2]] == 1] <- 2
  G[posOut, posRem][G[posOut, posRem] == 1] <- 2
  
  G[partners[,1], partners[,2]][G[partners[,1], partners[,2]] == 1] <- 2
  G[partners[,1], posRem][G[partners[,1], posRem] == 1] <- 2
  
  G[posOut, partners[,1]][G[posOut, partners[,1]] == 1] <- 2
  G[partners[,1], posOut][G[partners[,1], posOut] == 1] <- 2
  
  G[posRem, posBio][G[posRem, posBio] == 1] <- 2
  G[partners[,2], posBio][G[partners[,2], posBio] == 1] <- 2

  for (i in 1:nrow(partners)) {
    G[partners[i,1], partners[i,2]] <- 2
    G[partners[i,2], partners[i,1]] <- 3
  }
    
  # add unfaithful triples for upstream-instream counterparts where necessary
  tripleList <- triples(G, sepset, tripleList)
  ...
  res <- udag2pag(pag = G, sepset, rules = rules, unfVect = tripleList,
                  verbose = verbose, partners = partners)
...
# function that checks for conflicts between background knowledge and collider orientations

triples <- function(pag, sepset, unfVect = NULL) {
  ...
  for (i in seq_len(nrow(ind))) {
    ...
    for (z in allZ) {
      if (pag[x, z] == 0 && !((y %in% sepset[[x]][[z]]) ||
        ...
        # if there is a tail that the algorithm wants to orient as a collider, add it to list of unfaithful triples
        
        if (pag[z, y] == 3) {
            write("Triple", file = "data.txt", append = TRUE)
            unfVect <- c(unfVect, triple2numb(p, x, y, z))
        }
...
pdsep <- function (skel, suffStat, indepTest, p, sepset, alpha, pMax, m.max = Inf,
                   ...
                   amatUp = NULL) ## FIXME: verbose : 2 --> qreach(verbose)
{
  ...
  if (any(G)) {
    ...
    # for upstream case, function is finished here; else, continue with merged adjacency matrices
    if (is.null(amatUp)) {
      return(amat)
    } else {
      amat <- pmax(amat, amatUp)
    }
...
# changes to rule 4 to handle conflicts between background knowledge and rule
...
### while(.), the very first time already ??????????
            while (!Done && pag[a,b] != 0 && pag[a,c] != 0 && pag[b,c] != 0) {
              ...
              if ((N.md <- length(md.path)) == 1) {
                ...
                # if there is a conflict, next
                if ((a %in% partners[,2] & b %in% partners[,1])) {
                    if (partners[2, which(partners[,1] == b)] == a) {
                        next
                    }
                }
