...
# a function that perform bootstrapping
...
bootstrap <- function(data, nBoot, pBoot, indepTest, alpha,
  ...
  for (run in 1:nBoot) {
    ...
    # apply the algorithm
#    result <- flowFci(dataSet, indepTest, alpha, rownames(dataSet$C), partners = partners, posOut = posOut, posRem = posMisc, doPdsep = doPdsep, conservative = conservative, m.max = m.max, posBio = posBio)
    result <- pcalg::fci(dataSet, indepTest, alpha, rownames(dataSet$C), doPdsep = doPdsep, conservative = conservative, m.max = m.max)
  ...
  # remove the ones without a single count
...
flowFci <- function(suffStat, indepTest, alpha, labels, p,
                    ...
                    partners, posOut, posRem, posBio)
{
  ...
  # if two nodes were seperated in the first skeleton building, we have to copy the sepset to have a full set of them
  sepset <- skel@sepset
  ...
  if (doPdsep) {
    ...
    # as this is done for the instream variables, we have to delete it there and copy it upstream
    ...
    # now we call the full pdsep function which takes ad additional input the adjacency matrix that we learned for the upstream part
    # we use those for the other connections, but do not test the connections in themselves
    ...
    # we are not interested in the orientations upstream; they were only learned to be able to find the correct separating sets
  ...
  # add unfaithful triples for upstream-instream partners where necessary
                   ...
                   amatUp = NULL) ## FIXME: verbose : 2 --> qreach(verbose)
{
  ...
  if (any(G)) {
    ...
    # for upstream case, function is finished here; else, continue
    if (is.null(amatUp)) {
      return(amat)
    } else {
...
udag2pag <- function(pag, sepset, rules = rep(TRUE,10), unfVect = NULL, verbose = FALSE, orientCollider = TRUE, partners)
{
...
### while(.), the very first time already ??????????
            while (!Done && pag[a,b] != 0 && pag[a,c] != 0 && pag[b,c] != 0) {
              ...
              if ((N.md <- length(md.path)) == 1) {
                ...
                if ((a %in% partners[,2] & b %in% partners[,1])) { ### modified!!!
