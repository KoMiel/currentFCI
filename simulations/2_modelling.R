...
dir.create(path = paste0('models/simulations'), showWarnings = FALSE)
randomSeed <- c(seed, '\n')
...
foreach (noise = 1:6) %dopar% {
    dir.create(path = paste0('models/simulations', noise, "/"), showWarnings = FALSE)

    foreach (denseness = densenesses) %dopar% {
    dir.create(path = paste0('models/simulations', noise, '/', denseness, "/"), showWarnings = FALSE)
    
    foreach (nData = nDatas) %dopar% {
        dir.create(path = paste0('models/simulations', noise, '/', denseness, "/", nData, "/"), showWarnings = FALSE)
        
        foreach (alpha = alphas) %dopar% {
        dir.create(path = paste0('models/simulations', noise, '/', denseness, "/", nData, "/", alpha, "/"), showWarnings = FALSE)
